...
def extract_task(observation: str) -> str:
    ...
    if start < 0:
        raise ValueError("初始 observation 中没有找到 ALFWorld task 描述")
...
def _column(infos: dict[str, Any], key: str, size: int, default: Any) -> list[Any]:
    ...
    if isinstance(value, (list, tuple)):
        ...
        values = [value]
    if len(values) != size:
        raise ValueError(f"环境 infos[{key!r}] 长度 {len(values)} != batch size {size}")
    return values
...
class ALFWorldGroup:
    ...
    def reset(self) -> list[EnvironmentState]:
        ...
        resolved = {str(Path(value).expanduser().resolve()) for value in game_files}
        expected = str(self.example.game_file.resolve())
        if resolved != {expected}:
            raise RuntimeError(
                f"同组环境加载了错误游戏: expected={expected}, actual={sorted(resolved)}"
            )
    ...
    def step(self, actions: list[str]) -> list[EnvironmentStep]:
        ...
        if len(actions) != self.group_size:
            raise ValueError("每个 ALFWorld 分支都必须提供一个 action")
        normalized = [canonical_action(action) for action in actions]
        admissible = [
            action in current
            for action, current in zip(normalized, self._admissible_actions, strict=True)
        ]
        if self._uses_in_process_textworld:
            with _TEXTWORLD_SYNC_LOCK:
